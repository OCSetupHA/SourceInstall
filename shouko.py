...
class RobloxManager:
    ...
    def kill_roblox_process(package_name):
        ...
        while True:
            result = subprocess.run(f"ps -A | grep {package_name}", shell=True, capture_output=True, text=True)
            if package_name not in result.stdout and package_name not in result.stderr:
                print(f"\033[1;32m[ Shouko.dev ] - Process for {package_name} confirmed killed.\033[0m")
                return True
    ...
    def launch_roblox(package_name, server_link):
        ...
        with status_lock:
            ...
            globals()["package_statuses"][package_name]["Status"] = "\033[1;36mLaunching Roblox...\033[0m"
        ...
        while True:
            result = subprocess.run(f"ps -A | grep {package_name}", shell=True, capture_output=True, text=True)
            if package_name in result.stdout or package_name in result.stderr:
                with status_lock:
                    globals()["package_statuses"][package_name]["Status"] = "\033[1;32mJoined Roblox\033[0m"
                    UIManager.update_status_table()
                print(f"\033[1;32m[ Shouko.dev ] - Successfully launched and verified {package_name}.\033[0m")
                return
    ...
    def inject_cookies_and_appstorage():
        ...
        for package_name in packages:
            try:
                ...
                if not os.path.exists(destination_db_dir):
                    os.makedirs(destination_db_dir, exist_ok=True)
                if not os.path.exists(destination_appstorage_dir):
                    os.makedirs(destination_appstorage_dir, exist_ok=True)

                try:
                    pkg_stat = os.stat(f"/data/data/{package_name}")
                    uid = pkg_stat.st_uid
                    gid = pkg_stat.st_gid
                except OSError:
                    print(f"\033[1;31m[ Shouko.dev ] - Could not determine UID for {package_name}, using current.\033[0m")
                    uid = os.getuid()
                    gid = os.getgid()
                ...
                os.chown(destination_db_path, uid, gid)
                os.chmod(destination_db_path, 0o660)
                print(f"\033[1;32m[ Shouko.dev ] - Copied and set permissions for Cookies.db to {destination_db_path}\033[0m")
                ...
                os.chown(destination_appstorage_path, uid, gid)
                os.chmod(destination_appstorage_path, 0o660)
                print(f"\033[1;32m[ Shouko.dev ] - Copied and set permissions for appStorage.json to {destination_appstorage_path}\033[0m")
